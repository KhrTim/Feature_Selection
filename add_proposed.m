% add algs/proposed.m result to result/*.mat
clear;
output_dir = './result_prop/';
...
for k = 1:length(file_list)
    ...
    fprintf('%d Start\n', k);
    fea = load(strcat('./data/', file_list(k).name), 'fea').fea;
    gnd = load(strcat('./data/', file_list(k).name), 'gnd').gnd;
    for m = length(param_struct):length(param_struct)
